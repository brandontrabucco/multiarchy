...
class GoalConditioned(Relabeler):
    ...
    def relabel(
        ...
        if len(batch_of_data) == 4:
            ...
            goals = tf.reshape(observation["goal"], [batch_dim, max_path_length, -1])
            states = tf.reshape(
            ...
            observation[self.observation_key] = tf.concat(
                [observation[self.observation_key], *flatten(goals)], -1)
            return observation, actions, mixture, terminals

        elif len(batch_of_data) == 5:
            # the batch is using a step replay buffer notation
            observation, actions, rewards, next_observation, terminals = batch_of_data
            batch_dim = tf.shape(rewards)[0]

            goals = tf.reshape(observation["goal"], [batch_dim, -1])
            next_goals = tf.reshape(next_observation["goal"], [batch_dim, -1])
            ...
            observation[self.observation_key] = tf.concat(
                [observation[self.observation_key], *flatten(goals)], -1)
            next_observation[self.observation_key] = tf.concat(
                [next_observation[self.observation_key], *flatten(next_goals)], -1)
            return observation, actions, mixture, next_observation, terminals
